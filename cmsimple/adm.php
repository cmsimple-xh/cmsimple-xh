...
function selectlist($fn, $regm, $regr) {
    ...
    if($k1.$k2 == 'subsitetemplate')  // for subsite/language settings (template select box)
    {
...
// System Info and Help Links - GE 2010-10-28
    ...
    if ($f == 'sysinfo') {
        ...
        $o.= '<ul>' . "\n" . '<li>' . CMSIMPLE_XH_VERSION . '&nbsp;&nbsp;Released: ' . CMSIMPLE_XH_DATE . '</li>' . "\n" . '</ul>' . "\n" . "\n";
        ...
        if ($handle1) {
            ...
            while ($plugin1 = readdir($handle1)) {
                if (strpos($plugin1, '.') === false && $plugin1 != $pluginloader_cfg['foldername_pluginloader'] && is_dir($pth['folder']['plugins'] . $plugin1)) {
