 ...
 * @copyright 2009-2024 The CMSimple_XH developers <http://cmsimple-xh.org/?The_Team>
...
/**
  ...
  *
  */
if (isset($_GET['tinymce7']) && $_GET['tinymce7'] == 'imageuploader') {
    tinymce7_imageUploader();
    exit;
}
 ...
 */
    ...
    if (XH_wantsPluginAdministration($plugin)) {
        ...
        case 'plugin_main':           
            ...
            $o .= '<h1>TinyMCE7 for CMSimple_XH</h1>'.PHP_EOL;
            $o .= '<p>TinyMCE ';
            $o .= $plugin_cf['tinymce7']['CDN'] == true ? 'Content delivery network (CDN) ': '';
 ...
 */
function tinymce7_imageUploader()
{
    global $cf;
    
    $imgUploader = new Tinymce7\Uploader();
...
//         XH_logMessage( 'info', 'uploadFile', 'tinymce7', 'fileWritten: ' . $imgUploader->fileWritten);
      
    } else {
        foreach ($imgUploader->errMsg as $key => $val) {
            XH_logMessage( 'error', 'uploadFile', 'tinymce7', $key . ': ' . $val);
        }
